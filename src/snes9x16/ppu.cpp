...
#include "controls.h"
#include "display.h"
#ifdef FRANK_SNES_INDEXED_SCREEN
/* The framebuffer holds CGRAM indices; the colours go to the HDMI driver's
   palette instead. Both headers are C. */
extern "C" {
#include "graphics.h"
...
#ifdef FRANK_SNES_INDEXED_SCREEN
		/* The framebuffer carries CGRAM indices; the colours reach the
		   display through the HDMI palette instead. The push itself is
		   deferred - this runs from the $2100 write handler, i.e. in the
		   middle of a frame, and writing the palette while the driver is
		   scanning out tears the picture into bands. */
		IPPU.ScreenColors[i] = (uint16) i;
#else
...
#ifdef FRANK_SNES_INDEXED_SCREEN
	/* main.c pushes it at the next frame boundary. */
...
#ifdef FRANK_SNES_INDEXED_SCREEN
/* Hand CGRAM to the HDMI driver as a palette.
 *
 * Computed straight from CGDATA and the brightness table rather than from
 * IPPU.Red/Green/Blue: those are refreshed by two different paths (the
 * $2100 brightness handler and the $2122 CGRAM handler) and a push landing
 * between them picks up one channel updated and the others stale - which is
 * what turned the whole palette red.
 *
 * Only changed entries are written. graphics_set_palette_hdmi re-encodes a
 * TMDS pair on every call, and rewriting all 256  frame while the scanout
 * DMA is reading conv_color is enough to drop the HDMI signal.
 */
extern "C" void S9xPushPaletteToDisplay (void)
{
	/* Shadow the CGRAM word rather than the resulting colour: half the SRAM
	   and the same test, since the colour is a pure function of the pair. */
	static uint16_t shadow[256];
	static uint8_t  shadow_brightness = 0xff;
	const bool      valid = (shadow_brightness == (PPU.Brightness & 0x0f));

	const uint8 *xb = mul_brightness[PPU.Brightness & 0x0f];

	for (int i = 0; i < 256; i++)
	{
		const uint16 c = PPU.CGDATA[i];
		const uint32_t rgb = RGB888(xb[ c        & 0x1f] << 3,
		                            xb[(c >>  5) & 0x1f] << 3,
		                            xb[(c >> 10) & 0x1f] << 3);
	...
	shadow_brightness = PPU.Brightness & 0x0f;
}
#endif
