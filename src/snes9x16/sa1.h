...
struct SSA1
{
	...
	/* Device port: 32 KB of SA-1 memory map, in a build where SRAM is the
	   scarce resource and the render path is paying for it by living in
	   PSRAM. Allocated instead of declared - an SA-1 game gets a slower
	   map, every other game gets 32 KB back for the framebuffer. */
	uint8	**Map;
	uint8	**WriteMap;
#else
