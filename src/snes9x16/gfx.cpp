...
/* After the standard headers: redefining malloc before <cstdlib> breaks it. */
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
static uint16 *GFXScreenBuffer;   /* PSRAM; see S9xGraphicsInit */
#else
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
	/* Device port: sized for the native frame, not for Mode 7 hires 4x.
	   At 1024 px wide these five buffers come to 3.2 MB, and GFXScreenBuffer
	   was a std::vector - so it landed on the SDK's SRAM heap and panicked
	   the board with "out of memory" the moment a ROM started. Mode 7 hires
	   is off in this build (Settings.Mode7Hires stays 0), so the wide
	   geometry buys nothing here. malloc below is the PSRAM allocator. */
	GFX.Pitch      = sizeof(uint16) * SNES_WIDTH;
	GFX.RealPPL    = SNES_WIDTH;
	GFX.ScreenSize = SNES_WIDTH * SNES_HEIGHT_EXTENDED;
	tile_VRAM      = Memory.VRAM;
	tile_FillRAM   = Memory.FillRAM;
	GFXScreenBuffer = (uint16 *) malloc(sizeof(uint16) * SNES_WIDTH * (SNES_HEIGHT_EXTENDED + 64));
	if (!GFXScreenBuffer)
	{
		S9xGraphicsDeinit();
		return (FALSE);
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
	/* The main depth buffer stays in SRAM: the tile renderer touches it once
	   per pixel, exactly as often as the screen, and at 60 KB it is the
	   largest piece of the render path that actually fits. SubZBuffer is
	   only consulted for subscreen transparency, so it can afford PSRAM. */
	{
		static uint8 zbuf_sram[SNES_WIDTH * SNES_HEIGHT_EXTENDED]
			__attribute__((aligned(4)));
		GFX.ZBuffer = zbuf_sram;
	}
#else
