...
/* Every large buffer this file allocates - WRAM, VRAM, the tile caches,
   the ROM image - belongs in PSRAM, not in the 520 KB of SRAM the core
   and the drivers have to share. src/snes9x/memmap.c carries the same
   line for the 1.43 core. */
#include "../snes_alloc.h"
#define malloc snes_malloc
#endif

/* The ROM buffer is allocated per-ROM in this port, so clearing it with the
   MAX_ROM_SIZE ceiling overruns the allocation - and zeroes the ROM that was
   just loaded into it. */
#ifdef FRANK_SNES_CPU_CORE_S9X16
#define ROM_CLEAR_BYTES (Memory.ROMBufferBytes())
#else
#define ROM_CLEAR_BYTES (MAX_ROM_SIZE)
#endif
...
/* Every pointer that is derived from ROM or FillRAM. Extracted from Init()
   because this port allocates the ROM buffer per-ROM, after Init has run -
   so these have to be re-derived once it exists. Getting only some of them
   is how S9xInitBSX ended up reading BSXROMBase[0xFFDA] off a null base. */
...
{
	/* Device port: WRAM and VRAM were static arrays in the struct. They are
	   192 KB, which does not fit the RP2350's SRAM alongside everything
	   else, so they are allocated like every other large buffer here - the
	   port redirects malloc to the PSRAM allocator. */
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
	/* ROMStorage is NOT allocated here. At MAX_ROM_SIZE it would be 6.3 MB
	   of the board's 7.5 MB of usable PSRAM, which the tile caches and the
	   framebuffers also have to come out of. s9x16_alloc_rom() takes the
	   real ROM size once the front end knows it. */
	ROMStorage = NULL;
	ROMStorageSize = 0;
	SRAMStorageSize = SRAM_SIZE;
	SRAMStorage = (uint8_t *) malloc(SRAMStorageSize);
	if (!SRAMStorage)
	{
		Deinit();
		return (FALSE);
	}
	memset(SRAMStorage, 0, SRAMStorageSize);
	SRAM = &SRAMStorage[0];
#else
...
#endif
#ifdef FRANK_SNES_CPU_CORE_S9X16
	/* RAM and VRAM are pointers in this build - sizeof would be 4. */
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
	/* set by s9x16_alloc_rom once ROMStorage exists */
#else
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
	if (ROMStorage)  { free(ROMStorage);  ROMStorage  = NULL; }
	if (SRAMStorage) { free(SRAMStorage); SRAMStorage = NULL; }
#endif
...
#ifdef FRANK_SNES_CPU_CORE_S9X16
/* Device port: this is a human-readable ROM summary for a desktop UI, and
   the only thing in the build that uses <sstream>/<iomanip>. Those drag in
   libstdc++'s locale machinery, whose static initialiser
   (std::ctype<wchar_t>::_M_initialize_ctype) locks the board up before
   main() ever runs. Nothing on this device displays the string. */
