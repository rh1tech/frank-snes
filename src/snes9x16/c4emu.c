...
{
	return (C4ROMBase + ((Address & 0xff0000) >> 1) + (Address & 0x7fff));
}
...
static void C4ConvOAM (void)
{
	...
	OAMptr = C4RAMBase + (C4RAMBase[0x626] << 2);
	for (i = C4RAMBase + 0x1fd; i > OAMptr; i -= 4)
	...
	if (C4RAMBase[0x0620] != 0)
	{
		...
		offset = (C4RAMBase[0x626] & 3) * 2;
		srcptr = C4RAMBase + 0x220;

		for ( j = C4RAMBase[0x0620]; j > 0 && SprCount > 0; j--, srcptr += 16)
		{
...
static void C4DoScaleRotate (int row_padding)
{
	...
	if (READ_WORD(C4RAMBase + 0x1f80) == 0)	/* no rotation */
	{
	...
	if (READ_WORD(C4RAMBase + 0x1f80) == 128)	/* 90 degree rotation */
	{
		/* XXX: Really do this? */
		A = 0;
		B = (int16_t) (-YScale);
	...
	if (READ_WORD(C4RAMBase + 0x1f80) == 256)	/* 180 degree rotation */
	{
		/* XXX: Really do this? */
		A = (int16_t) (-XScale);
		B = 0;
	...
	if (READ_WORD(C4RAMBase + 0x1f80) == 384)	/* 270 degree rotation */
	{
		/* XXX: Really do this? */
		A = 0;
		B = (int16_t) YScale;
	...
	{
		A = (int16_t)   SAR(C4CosTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * XScale, 15);
		B = (int16_t) (-SAR(C4SinTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * YScale, 15));
		C = (int16_t)   SAR(C4SinTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * XScale, 15);
		D = (int16_t)   SAR(C4CosTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * YScale, 15);
	}

	/* Calculate Pixel Resolution */
	w = C4RAMBase[0x1f89] & ~7;
	h = C4RAMBase[0x1f8c] & ~7;

	/* Clear the output RAM */
	memset(C4RAMBase, 0, (w + row_padding / 4) * h / 2);

	Cx = (int16_t) READ_WORD(C4RAMBase + 0x1f83);
	Cy = (int16_t) READ_WORD(C4RAMBase + 0x1f86);
	...
	for ( y = 0; y < h; y++)
	{
		...
		for ( x = 0; x < w; x++)
		{
			...
			/* De-bitplanify */
			if (byte & 1)
				C4RAMBase[outidx]      |= bit;
			if (byte & 2)
				C4RAMBase[outidx + 1]  |= bit;
			if (byte & 4)
				C4RAMBase[outidx + 16] |= bit;
			if (byte & 8)
				C4RAMBase[outidx + 17] |= bit;
...
{
	uint8_t	*line = C4GetMemPointer(READ_3WORD(C4RAMBase + 0x1f80));
	...
	for ( i = C4RAMBase[0x0295]; i > 0; i--, line += 5)
	{
		if (line[0] == 0xff && line[1] == 0xff)
		{
			...
			while (tmp[2] == 0xff && tmp[3] == 0xff)
				tmp -= 5;
			point1 = C4GetMemPointer((C4RAMBase[0x1f82] << 16) | (tmp[2]  << 8) |  tmp[3]);
		}
		else
			point1 = C4GetMemPointer((C4RAMBase[0x1f82] << 16) | (line[0] << 8) | line[1]);

		point2 = C4GetMemPointer((C4RAMBase[0x1f82] << 16) | (line[2] << 8) | line[3]);
...
static void C4BitPlaneWave (void)
{
	...
	uint8_t	*dst = C4RAMBase;
	uint32_t	waveptr = C4RAMBase[0x1f83];
	...
	for ( j = 0; j < 0x10; j++)
	{
		do
		{
			int16_t	height = -((int8_t) C4RAMBase[waveptr + 0xb00]) - 16;
...
static void C4SprDisintegrate (void)
{
	...
	Cx = (int16_t) READ_WORD(C4RAMBase + 0x1f80);
	Cy = (int16_t) READ_WORD(C4RAMBase + 0x1f83);


	scaleX = (int16_t) READ_WORD(C4RAMBase + 0x1f86);
	scaleY = (int16_t) READ_WORD(C4RAMBase + 0x1f8f);
		...
		for ( x = StartX, j = 0; j < width; j++, x += scaleX)
		{
			if ((x >> 8) < width && (y >> 8) < height && (y >> 8) * width + (x >> 8) < 0x2000)
			{
				...
				if (pixel & 1)
					C4RAMBase[idx]      |= mask;
				if (pixel & 2)
					C4RAMBase[idx + 1]  |= mask;
				if (pixel & 4)
					C4RAMBase[idx + 16] |= mask;
				if (pixel & 8)
					C4RAMBase[idx + 17] |= mask;
			}
...
uint8_t S9xGetC4 (uint16_t Address)
{
	...
	return (C4RAMBase[Address - 0x6000]);
}
...
{
   /* Square of the signed 24-bit value at $1f80, 48-bit result.
    * The value must be sign-extended from bit 23 before squaring;
    * done manually to avoid shifting into the sign bit (UB). The
    * previous chained-SAR form squared the value pre-shifted left
    * by 30 bits, overflowing int64 and always writing zero. */
   int64_t a = (int64_t) READ_3WORD(C4RAMBase + 0x1f80);
...
void S9xSetC4 (uint8_t byte, uint16_t Address)
{
		...
		else
		{
			switch (byte)
			{
				...
				case 0x05: /* Propulsion (?) */
				{
					...
					WRITE_WORD(C4RAMBase + 0x1f80, (uint16_t) tmp);
					break;
				}
				...
				case 0x10: /* Polar to rectangular */
				{
					int32_t	tmp;
					int32_t   r1;
					r1 = READ_WORD(C4RAMBase + 0x1f83);
					if (r1 & 0x8000)
					        r1 |= ~0x7fff;
					else
					        r1 &= 0x7fff;				
										
					tmp = SAR(r1 * C4CosTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * 2, 16);
					WRITE_3WORD(C4RAMBase + 0x1f86, tmp);
					tmp = SAR(r1 * C4SinTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * 2, 16);
				...
				{
					int32_t	tmp;
					tmp = SAR((int32_t) READ_WORD(C4RAMBase + 0x1f83) * C4CosTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * 2, 8);
					WRITE_3WORD(C4RAMBase + 0x1f86, tmp);
					tmp = SAR((int32_t) READ_WORD(C4RAMBase + 0x1f83) * C4SinTable[READ_WORD(C4RAMBase + 0x1f80) & 0x1ff] * 2, 8);
				...
				case 0x22: /* Trapezoid */
				{
					int16_t	angle1 = READ_WORD(C4RAMBase + 0x1f8c) & 0x1ff;
					...
					for ( j = 0; j < 225; j++)
					{
						if (y >= 0)
						{
							left  = SAR((int32_t) tan1 * y, 16) - READ_WORD(C4RAMBase + 0x1f80) + READ_WORD(C4RAMBase + 0x1f86);
							right = SAR((int32_t) tan2 * y, 16) - READ_WORD(C4RAMBase + 0x1f80) + READ_WORD(C4RAMBase + 0x1f86) + READ_WORD(C4RAMBase + 0x1f93);
						...
						C4RAMBase[j + 0x800] = (uint8_t) left;
						C4RAMBase[j + 0x900] = (uint8_t) right;
				...
				case 0x25: /* Multiply */
				{
					int32_t	foo = READ_3WORD(C4RAMBase + 0x1f80);
					int32_t	bar = READ_3WORD(C4RAMBase + 0x1f83);
					foo *= bar;
	...
	if (Address == 0x7f47)
	{
		memmove(C4RAMBase + (READ_WORD(C4RAMBase + 0x1f45) & 0x1fff), C4GetMemPointer(READ_3WORD(C4RAMBase + 0x1f40)), READ_WORD(C4RAMBase + 0x1f43));
	}
}
...
uint8_t * S9xGetMemPointerC4 (uint16_t Address)
{
	if (Address >= 0x7f40 && Address <= 0x7f5e)
		return (NULL);
	return (C4RAMBase - 0x6000 + (Address & 0xffff));
}
