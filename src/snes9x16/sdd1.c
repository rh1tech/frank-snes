...
void S9xSetSDD1MemoryMap (uint32_t bank, uint32_t value)
{
	...
	{
		uint8_t	*block = &BridgeROM[sdd1_mirror(BridgeCalculatedSize, value + (c << 12))];
		for ( i = c; i < c + 16; i++)
			BridgeMap[i + bank] = block;
	}
}
...
{
	int i;
	memset(&BridgeFillRAM[0x4800], 0, 4);
	for ( i = 0; i < 4; i++)
	{
		BridgeFillRAM[0x4804 + i] = i;
...
void S9xSDD1PostLoadState (void)
{
	int i;
	for ( i = 0; i < 4; i++)
		S9xSetSDD1MemoryMap(i, BridgeFillRAM[0x4804 + i]);
}
