...
/* ppu.h is included from the C coprocessor sources too, so the linkage
   specifier has to be conditional. */
#ifdef __cplusplus
extern "C" volatile bool g_palette_needs_update;
#else
extern volatile _Bool g_palette_needs_update;
#endif
#endif

/* FRANK_SNES_INDEXED_SCREEN: this device scans out 8-bit palette-indexed
   pixels - the HDMI driver holds the palette and the framebuffer holds
   CGRAM indices. Every pixel the tile renderer writes goes through
   GFX.ScreenColors[], so making that table the identity is enough to turn
   the whole renderer's output from colour into index, without touching
   tile.c at all. The palette itself reaches the display from CGRAM.

   The cost is colour maths: adding or subtracting two indices is
   meaningless. This port already gave that up - it builds with
   SIMPLE_COLOR_MATH and NO_ZERO_LUT for the same reason. */
...
#ifdef FRANK_SNES_INDEXED_SCREEN
			IPPU.ScreenColors[PPU.CGADD] = (uint16) PPU.CGADD;
			/* The colour itself only reaches the screen through the
			   display palette now, so a CGRAM write has to mark it. */
